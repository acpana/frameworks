...
func BenchmarkClient_Review(b *testing.B) {
	...
	for _, templates := range []int{1, 10, 100} {
		for _, constraints := range []int{1, 5, 10, 50, 100, 500, 1000, 10000} {
...
func BenchmarkClient_Review_multipleObjects(b *testing.B) {
	tests := []struct {
		name           string
		review         handlertest.Review
		makeConstraint func(tid int, name string) *unstructured.Unstructured
	...
	for _, numObjects := range []int{1, 5, 10, 100, 500, 1000} {
		for _, templates := range []int{1, 10, 100} {
			for _, constraints := range []int{1, 5, 10, 50, 100, 500, 1000, 10000} {
				...
				for _, tt := range tests {
					...
					b.Run(fmt.Sprintf("%d NumObjectsMatched %d Constraints %d Templates %s", numObjects, constraints, templates, tt.name), func(b *testing.B) {
						b.ResetTimer()
						// Run Review queries in parallel.
						b.RunParallel(func(pb *testing.PB) {
							for pb.Next() {
